...
import (
	...
	"github.com/xiehqing/hiagent/internal/oauth"
	"github.com/xiehqing/hiagent/internal/oauth/copilot"
)
...
type DatabaseOptions struct {
	Driver string `json:"driver,omitempty" jsonschema:"description=Database driver to use for Crush state,enum=sqlite,enum=mysql,default=sqlite,example=mysql"`
	DSN    string `json:"dsn,omitempty" jsonschema:"description=Database connection DSN used when driver is mysql,example=user:password@tcp(127.0.0.1:3306)/crush?parseTime=true&charset=utf8mb4"`
}
...
type Options struct {
	...
	Database                  *DatabaseOptions `json:"database,omitempty" jsonschema:"description=Database backend configuration used for Crush state storage"`
